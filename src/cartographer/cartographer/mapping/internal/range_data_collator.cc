...
sensor::TimedPointCloudOriginData RangeDataCollator::AddRangeData(
  ...
  if (id_to_pending_data_.count(sensor_id) != 0) {    //利用count(id)来检测对应id雷达是否有待处理数据
    ...
    // 保存当前点云
    id_to_pending_data_.emplace(sensor_id, std::move(timed_point_cloud_data));   //在进行Crop之后，再将当前传入的点云数据传入待处理数聚集
    return result;
  }
...
sensor::TimedPointCloudOriginData RangeDataCollator::CropAndMerge() {
  ...
  for (auto it = id_to_pending_data_.begin();
    ...
    auto overlap_begin = ranges.begin();   //overlap_begin是range中每一个点的迭代器
    while (overlap_begin < ranges.end() &&
           data.time + common::FromSeconds((*overlap_begin).time) <   //common... 中为每一个点的局部时间
               current_start_) {                                             //将他加上data.time 则将其获得到全局时间
      ++overlap_begin;
    }
    ...
    if (overlap_begin < overlap_end) {
      ...
      //push.back 当且仅当新的vector大小超过当前vector容量(capacity)时，会重新自动分配新的存储空间。这时iterator会失效
      //利用reserve可以直接修改vector的capacity
